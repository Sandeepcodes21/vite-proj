...
const VALID_COURSES = ['RDBMS','CO','STATISTICS','DSA'];
const SemesterFour = () => {
  const [subject, setSubject] = useState("");
  const [file, setFile] = useState(null);
  const [sets, setSets] = useState(0);
  const [vsq, setVsq] = useState(0);
  const [sq, setSq] = useState(0);
  const [longQ, setLongQ] = useState(0);
  const [caseQ, setCaseQ] = useState(0);
  ...
  useEffect(() => {
    document.title = "Semester 4"; // Set the tab name to "Semester 1"
  }, []);
